...
function Get-ComposeCommand {
    ...
    if ($null -ne $docker) {
        ...
        try {
            ...
            # PowerShell 5.1: avoid 2>$null which prevents $LASTEXITCODE
            # from being set. Capture stderr into a variable instead.
...
function Invoke-StartFaults {
    ...
    # Pass 1: group netem faults by container target so multiple netem clauses
    # are merged into a single `tc qdisc replace` call. (Original script
    # called `tc qdisc replace` once per fault, so a delay+loss pair on the
    # same target had its first qdisc overwritten by the second.)
    $netemPlans = [ordered]@{}
    $orderedActions = @()
    foreach ($fault in $Faults) {
        ...
        switch ($action) {
            "netem_delay" {
                if (-not $netemPlans.Contains($target)) {
                    $netemPlans[$target] = [ordered]@{
                        delay_ms = $null
                        jitter_ms = $null
                        loss_percent = $null
                        events = @()
                ...
                $netemPlans[$target].delay_ms = [int]$fault.delay_ms
                $netemPlans[$target].jitter_ms = [int]$fault.jitter_ms
                $netemPlans[$target].events += [ordered]@{
                    action = $action
                    detail = "delay=$([int]$fault.delay_ms)ms jitter=$([int]$fault.jitter_ms)ms"
                }
            }
            "netem_loss" {
                if (-not $netemPlans.Contains($target)) {
                ...
                $netemPlans[$target].loss_percent = [double]$fault.loss_percent
                $netemPlans[$target].events += [ordered]@{
                    action = $action
                    detail = "loss=$([double]$fault.loss_percent)%"
                }
            }
            "pause_container" {
    ...
    # Pass 2: emit one combined netem clause per target.
    foreach ($target in $netemPlans.Keys) {
        $plan = $netemPlans[$target]
        $parts = @()
        if ($null -ne $plan.delay_ms) {
            $parts += "delay $($plan.delay_ms)ms $($plan.jitter_ms)ms"
        }
        if ($null -ne $plan.loss_percent) {
            $parts += "loss $($plan.loss_percent)%"
        }
        $clause = $parts -join " "
        & docker exec $target sh -c "tc qdisc replace dev eth0 root netem $clause" > $null
        if ($LASTEXITCODE -ne 0) {
            throw "combined netem failed for $target ($clause)"
        }
        foreach ($ev in $plan.events) {
            Add-FaultEvent -TimelinePath $timeline -RunId $RunId -EventType "fault_applied" -Action $ev.action -Target $target -Status "applied" -Detail "$($ev.detail) combined=[$clause]"
        }
    }

    # Pass 3: apply pause_container faults (independent of qdisc).
    foreach ($entry in $orderedActions) {
        if ($entry.kind -ne "pause") { continue }
        $fault = $entry.fault
        $target = $entry.target
        ...
        Add-FaultEvent -TimelinePath $timeline -RunId $RunId -EventType "fault_applied" -Action "pause_container" -Target $target -Status "paused" -Detail "duration=${durationSeconds}s"
...
foreach ($entry in $plan) {
    ...
    try {
        ...
        if ($StartCompose) {
            ...
            # Run docker-compose directly; save exit code immediately.
            # docker-compose writes to stderr which PowerShell wraps as
            # ErrorRecord; capture all output then check exit code.
            $prevEAP = $ErrorActionPreference
            $ErrorActionPreference = "Continue"
            $composeOutput = & docker-compose -f $composeFile up -d 2>&1
            $composeExit = [int]$LASTEXITCODE
            $ErrorActionPreference = $prevEAP
            Write-Output ($composeOutput | Out-String)
            if ($composeExit -ne 0) {
                throw "compose up failed for $runId (exit=$composeExit)"
            }
            $composeStarted = $true
            ...
            # Capture fullnode log immediately while container is still alive.
            # (collect_container_state runs later in finally, by which point the
            # fullnode may have exited; this early capture guarantees the log.)
            $fullnodeContainer = "${runId}-fullnode"
            $earlyLogDir = Join-Path $runRoot "evidence\early_logs"
            New-Item -ItemType Directory -Force $earlyLogDir | Out-Null
            $earlyLogPath = Join-Path $earlyLogDir "${fullnodeContainer}.logs.txt"
            try {
                $logLines = & docker logs --timestamps $fullnodeContainer 2>&1
                $logOutput = ($logLines | Out-String)
                Set-Content -Path $earlyLogPath -Value $logOutput -Encoding UTF8
                if ($LASTEXITCODE -ne 0 -and $null -ne $LASTEXITCODE) {
                    Write-Warning "early fullnode log capture failed for $fullnodeContainer (exit=$LASTEXITCODE)"
                }
            }
            catch {
                Write-Warning "early fullnode log capture exception: $($_.Exception.Message)"
            }
        }
