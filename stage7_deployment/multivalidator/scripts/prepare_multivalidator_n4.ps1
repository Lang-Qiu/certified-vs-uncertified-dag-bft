param(
    [string]$RunId = "",
    [int]$Seed = 20260525,
    [string]$Scenario = "baseline_n4",
    [int]$RepeatIndex = 1,
    [int]$EpochDurationMs = 60000,
    [string]$SuiImage = "stage7-sui:local"
)

# n=4 variant of prepare_multivalidator.ps1. Differences from n=7:
#   - committee-size 4 (instead of 7)
#   - 4 benchmark IPs on subnet 172.28.4.0/24 (instead of 172.28.7.0/24)
#   - --validators 4 --fault-tolerance 1 (BFT n=3f+1, f=1)
#   - expected genesis artifacts list reduced to 4 validators
# Everything else (fullnode patching, sui genesis invocation, log layout) is identical.
...
function Patch-FullnodeNetworkConfig {
    ...
    $fullnodeNetworkAddress = "network-address: /ip4/172.28.4.20/tcp/2070/https"
    $fullnodeListenAddress = 'listen-address: "0.0.0.0:2071"'
    $fullnodeExternalAddress = "external-address: /ip4/172.28.4.20/udp/2071"
    ...
    foreach ($expected in @(
        $fullnodeNetworkAddress, $fullnodeListenAddress,
        $fullnodeExternalAddress, $fullnodeMetricsAddress
    )) {
...
$expectedFiles = @(
    "172.28.4.11-2000.yaml",
    "172.28.4.12-2010.yaml",
    "172.28.4.13-2020.yaml",
    "172.28.4.14-2030.yaml",
    "network.yaml",
    "fullnode.yaml",
    "genesis.blob"
)
