...
% superscripts/greek introduced by the cost-model coupling & RBC analyses
...
§7.6.1)。须强调本文的部署组件定位为成本模型参数的\textbf{结构性标定}(structural
calibration),而非两套独立协议的 benchmark
比较:其 certified 对照臂由同一引擎上一个\textbf{可开关的代理认证}(proxy-certified
toggle,以 2f+1 quorum 接受模拟认证,而非密码学 RBC)实现,故标定的是净优势的成本结构(Δ\_save、翻转频率 ρ*),而非真实 RBC 的绝对延迟;真实 RBC
开销的解析外推见 §7.6.4,构念效度限定见 §9。
...
a non-linear, fault-intensity-driven contingent liability. We stress
that the deployment component is positioned as a structural calibration
of the cost-model parameters, not a benchmark of two independent
protocols: its certified baseline is realized by a toggleable
proxy-certified gate (an availability certificate accepting at a 2f+1
quorum, not cryptographic RBC) on a single engine, so it calibrates the
cost structure of the net advantage (Δ\_save and the crossover frequency
ρ*) rather than the absolute latency of real RBC; an analytical
extrapolation to real RBC cost is given in §7.6.4 and construct-validity
limits in §9.
...
须澄清本文的批判对象与定位,以免被误读为攻击一个稻草人。本文并不主张协议设计社区\textbf{完全意识不到}去认证有代价------恰恰相反,从业者在非正式讨论中早已知道
uncertified DAG 需要额外的可用性与恢复机制,Adelie {[}45{]}
为其新攻击面专门设计检测与防护即是一例,Mysticeti {[}43{]}
自身也讨论过认证的取舍。本文要做的不是``纠正一个错误的共识'',而是把这种\textbf{分散的、非正式的、随协议各异的}认识\textbf{系统化并加以扩展}:去认证的代价究竟落在哪一个不变量上?以何种成本结构(固定开销还是或有负债)出现?在何种网络与敌手条件下足以抵消良好情形的延迟优势?这些问题此前从未被作为一个\textbf{受控的分析对象}统一追问。换言之,本文把一个被
benchmark
叙述所遮蔽、半已知的取舍,提升为一个可分析、可证伪、且有部署锚点的框架------这正是其增量贡献所在。

本文质疑``去认证免费降延迟''这一\textbf{结构性}表述,质疑的起点是一个方法论问题。本文记之为
...
uncertified
只是这一深层轴上的两个投影点。第 8 章将进一步把贡献 C 的\textbf{可辩护内核}收窄为其中的
reconciliation trigger
维度,以及``延迟与鲁棒性是二维平面上可联合优化的目标,而非一维取舍''这一设计自由度------这正是不可被归为``对二分法的重命名''的部分。
...
综观上述谱系,已有工作的主流范式是``提出一个新协议并报告其 benchmark''。系统化综述虽已出现,但其分类坐标均非本文所关注的认证强度轴,值得逐一对照。Cachin 与 Vukolić 对实用 BFT 协议的早期梳理 {[}60{]} 以网络模型、容错门限与通信复杂度为骨架;Bano 等 {[}58{]} 以委员会形成机制与 Sybil 抗性来组织区块链共识谱系;Garay 与 Kiayias {[}59{]} 给出一个以网络/敌手假设与安全活性属性为维度的形式化共识分类法;而 Raikwar 等针对 DAG 共识的专门 SoK {[}57{]} 则按 DAG 的结构形态(结构化/非结构化)、轮与波的推进方式以及提交规则来归类协议。这四类分类法各有价值,却有一个共同点:它们都把 certified 与 uncertified 视为已有协议的\textbf{既定属性}加以登记和归类,而\textbf{没有一篇}把``certified/uncertified 这条分界线本身''当作分析对象、追问它是否遮蔽了一个更恰当的坐标。Mysticeti {[}43{]}、Adelie {[}45{]} 等单个协议工作确实讨论过认证的取舍,但它们讨论的是``在这条轴上如何取值'',而非``这条轴本身是否是描述设计空间的恰当坐标''。本文要填补的正是后一种 reframing 的空白------它与上述 SoK 是\textbf{正交而非竞争}的:本文不提出新的协议分类法,而是论证现有分类法所共同沿用的一条分界线(认证强度)只是一个更深层变量(availability-enforcement 时机与 reconciliation trigger,见第 8 章)的粗糙投影。

须就语料库 C(§3.3.1)的样本范围作一诚实说明。本文的归纳样本止于 2024--2025 年间发表的七个协议;DAG-BFT 仍是一个快速演进的领域,持续涌现的新设计可按 C 的相同纳入标准(有公开安全性证明、有明确提交规则)被纳入并定位到第 8 章的深层轴上。本文不宣称已穷尽该设计空间,这一持续扩充属于未来工作(§10);但归纳样本的有限性不影响本文结论的内部效度------第 6 章命题 1 对``安全性证明依赖槽位唯一性''的协议类给出的是不依赖逐协议归纳的论证,而第 5 章的 ①/② 分类对 C 内每个协议均经 C1-C 双判据独立核验。
...
须坦承归纳样本之小及其多样性之低:C 中仅有四个 uncertified
协议(Cordial Miners、Mysticeti、Mahi-Mahi、Adelie),且其中三个(Mysticeti、Mahi-Mahi、Adelie)出自相近的研究脉络------Mahi-Mahi
与 Adelie 均构建于或紧邻 Mysticeti 的 uncertified
结构。这意味着第 5 章对 uncertified 一侧的归纳结论(可用性落入 ②
削弱+补偿)在统计意义上是一个\textbf{弱归纳}:它的外部效度不应依赖样本量,而依赖两道独立于样本量的支撑------第
6 章命题 1 对非等价性给出的、不依赖逐协议归纳的论证,以及对 C
中每个协议安全性/活性证明的逐一核验(C1-C
双判据,见第 5 章)。扩大 uncertified 样本(纳入更多独立脉络的协议)以确认该归纳,是一项明确的未来工作(§10)。
...
须就术语作一消歧,以免与区块链可扩展性文献中的同名概念混淆。本文所称``数据可用性''是一个\textbf{共识层}性质------指被引用区块的内容\textbf{确实被足够多的诚实验证者持有},从而可在需要时被取得;它刻画的是``数据是否真的存在于网络中''这一物理事实。这与数据可用性采样(DAS)及欺诈/可用性证明
{[}48{]} 文献中的``数据可用性''\textbf{不同}:后者关心的是\textbf{轻客户端}如何在不下载全量数据的前提下\textbf{验证}区块数据已被发布(经纠删码加随机抽样),其威胁模型是``发布者隐瞒数据''。两者相关却不等同------本文的可用性是``数据被持有''这一事实\textbf{本身},而
DAS 是``如何低成本地确信该事实''的\textbf{检测层}。本文全文在前一意义上使用该术语;下文为行文简洁,``可用性''(availability)与``数据可用性''互为简称、同指此共识层不变量,不再区分。
...
须正视逻辑型/物理型边界在此处的一个\textbf{边缘情况}。说因果历史完整性``可仅凭本地结构检查验证'',严格而言带有一个\textbf{数据前置条件}:要检查某个祖先是否良构(签名正确、引用无环),验证者必须\textbf{已经持有}该祖先区块------没有数据,结构检查无从谈起。因此逻辑型与物理型并非全然正交:逻辑型不变量的验证以物理型可用性为\textbf{前提}。本文仍把因果历史完整性归为逻辑型,理由在于其判据所\textbf{要求的计算}是纯本地、确定性的------给定数据即可判定,无需任何网络交互,亦无需推断``某份数据是否真实存在于别处''这一不可由本地检查得出的物理事实;而数据是否到手,本就是另一个不变量(数据可用性)的职责。换言之,二分的恰当表述是:逻辑型不变量的\textbf{验证逻辑}不依赖物理事实,但其\textbf{执行}消费由物理型不变量供给的数据。这一分层非但不削弱第
4.5
节的异质性论点,反而强化了它------正因为逻辑型验证把``数据是否可得''外包给了可用性不变量,去认证后逻辑型才能低成本平移(平移时不必自带数据保证),而所有物理代价都被干净地归集到可用性这一个物理型不变量上(§5.3
末``因果历史完整性的平移不偷渡物理代价''即此分层的直接推论)。
...
为使命题 1 论证的前提可核查,在此把敌手能力\textbf{显式}列为四条假设:

\begin{itemize}
\tightlist
\item
  \textbf{(A1)静态拜占庭腐化}:敌手在执行开始前腐化至多 f
  个验证者(n≥3f+1);被腐化者可任意偏离协议,特别是可对同一 (author,
  round) 槽位制造两个不同的等价区块。
\item
  \textbf{(A2)调度全权(rushing adversary)}:敌手完全控制消息的投递顺序与时延(无上界但保证最终送达),且是\textbf{抢先}的------在决定何时、向哪个正确验证者、投递哪条消息之前,敌手可先观察当轮所有已发送消息的内容。
\item
  \textbf{(A3)网络异步}:不存在已知的消息延迟上界,正确验证者之间无同步时钟;敌手据(A2)可使不同正确验证者在任一有限时刻持有\textbf{相互不一致}的本地
  DAG 视图。
\item
  \textbf{(A4)确定性本地决策}:每个正确验证者的提交决定是其当前本地 DAG
  的确定性函数,不含私有随机性(若协议使用公共币,则论证对最坏 coin
  取值成立)。
\end{itemize}

§6.2 的反证构造只动用 (A1)--(A3):rushing
调度(A2)恰是``把 X 投给 v₁、把 X′ 投给
v₂''得以实现的能力,异步(A3)则保证``该视图不一致可在安全性必须成立的有限时刻持续存在''。命题
1 的条件前提(``安全性证明依赖槽位唯一性'')与这四条假设共同界定了论证的适用范围。
...
\textbf{槽位唯一性依赖在实践中能否被打破?}
命题 1 是\textbf{条件性}的------它只约束``安全性证明依赖槽位唯一性''的协议类。一个自然的追问是:这一条件前提在实践中是否可能被规避,从而架空命题
1?本文的评估是:在现有 DAG-BFT 范式内,规避它极为困难。原因在于 DAG
共识的全序映射把 (author, round) 槽位当作全序中的\textbf{原子寻址单位}------``第
r 轮、作者 a
的那个区块''是排序规则所寻址的基本对象;只要排序以这种``按槽位寻址''的方式进行,槽位的多重占据就直接破坏全序的良定义性,安全性证明便\textbf{必然}要在某处依赖``每槽位至多一个''。要真正绕开槽位唯一性,需要一种\textbf{不以
author-round 槽位为寻址单位}的排序抽象(例如纯内容寻址,或允许同槽位多区块共存、在更高层去重的设计),而语料库
C 乃至本文所知的全部已部署 DAG-BFT
协议都未采用这类抽象。因此,命题 1
虽形式上是条件性的,其条件前提在实践中近乎\textbf{普适}------这正是它实际意义之所在:它并非一个适用面狭窄的技术命题,而是堵死了``在主流
DAG-BFT 范式内干脆消除非等价性''这条退路。
...
\subsubsection{7.4.1 ρ 与 Δ\_recover
的耦合:一个敏感性分析}\label{sec:coupling-sensitivity}

§7.2 把 ρ 与 Δ\_recover
当作独立变量,从而得到一个良定义的标量阈值 ρ* = Δ\_save /
Δ\_recover。本节的拥塞反馈正是这一独立性最可能失效的场景:恢复请求本身占用带宽,触发越频繁、单次恢复越贵------ρ 与 Δ\_recover
被共同驱动而正相关。本节给出耦合情形下的代数分析,并刻画一个使耦合最大化的对抗性情景,以定量界定独立性假设的适用边界。

\textbf{线性耦合模型。}
把拥塞反馈建模为 Δ\_recover 对 ρ 的线性依赖:Δ\_recover(ρ) =
Δ\_recover⁰ · (1 + κρ),其中 Δ\_recover⁰ 为零耦合时的基础恢复成本,κ ≥ 0
为耦合系数(κ=0 即退回 §7.2 的独立情形)。代入净优势公式:

净优势(ρ) ≈ Δ\_save − ρ · Δ\_recover⁰ · (1 + κρ) = Δ\_save −
Δ\_recover⁰ · ρ − κ · Δ\_recover⁰ · ρ²。

原本对 ρ 线性的净优势因此变为一条开口向下的抛物线:二次项 −κ·Δ\_recover⁰·ρ²
使净优势随 ρ 上升而\textbf{加速}下跌。令净优势=0,解出耦合情形下的临界频率

$$\rho^{*}_{\mathrm{coupled}} = \frac{-1 + \sqrt{\,1 + 4\kappa\,\Delta_{\mathrm{save}}/\Delta_{\mathrm{recover}}^{0}\,}}{2\kappa}.$$

两个性质值得指出。其一(一致性):当 κ→0 时,对上式用
$\sqrt{1+x}\approx 1 + x/2$ 展开即得 $\rho^{*}_{\mathrm{coupled}} \to
\Delta_{\mathrm{save}}/\Delta_{\mathrm{recover}}^{0} =
\rho^{*}_{\mathrm{indep}}$,耦合模型连续地退回独立模型。其二(单边收缩):对任意
κ\textgreater0,由严格不等式 $\sqrt{1+x} < 1 + x/2$(x\textgreater0
时成立)可证 $\rho^{*}_{\mathrm{coupled}} <
\rho^{*}_{\mathrm{indep}}$------\textbf{耦合总是把翻转点推向更低的
ρ},即缩小 uncertified
的占优区。直观地说,正反馈使隐性代价比独立模型预测的更早压过 Δ\_save。

\textbf{对抗性情景。}
这一代数结果有一个直接的安全含义。一个自适应敌手不必盲目提高故障率
ρ;它可以\textbf{择时}------在网络已经拥塞、Δ\_recover⁰
已偏高的时刻集中制造数据缺失,从而最大化有效
κ。换言之,敌手通过把``故障频率''与``单次恢复成本''人为正相关起来,迫使系统沿抛物线的陡峭段运行。由此可得一个对评测有意义的推论:独立模型(κ=0)是对
uncertified 一侧\textbf{最有利}的假设;任何现实的拥塞反馈或择时敌手,都只会使真实的翻转频率\textbf{低于}
§7.6.3 在独立假设下标定的 ρ* 值。

\textbf{量级。} 这一收缩并非可忽略的二阶修正。以 §7.6.3 标定的独立阈值
$\rho^{*}_{\mathrm{indep}} \approx 0.038$ 为基准,代入一个中等耦合
κ=50(即 ρ 每升高 0.02,单次恢复成本即翻倍),得 $x =
4\kappa\,\rho^{*}_{\mathrm{indep}} \approx 7.6$,$\rho^{*}_{\mathrm{coupled}}
\approx (-1+\sqrt{8.6})/100 \approx 0.019$------临界频率近乎减半。即在敌手择时或重拥塞下,uncertified
的占优区可被实质性压缩到独立估计的一半左右。

\textbf{与实证的呼应。} §7.6.3 的``诚实模型偏离''正是这一耦合的部署指纹:当以单窗
amortized cadence 直接测净优势时,净优势对 Δ\_recover
呈非单调(线性拟合 R²≈0.02),因为在极高 Δ\_recover 下 certificate-gate
会延长地排斥仍在恢复、尚未被 quorum
接受的落后节点------这是一个使 Δ\_recover
与有效触发频率相互纠缠的竞争效应,即 κ\textgreater0
的经验显影。因此独立性假设的失效不是一个纯理论顾虑,而是已在 §7.6.3
的极端区被观测到。综上,§7.2 中``ρ* 为良定义标量''的表述,应严格限定为
κ≈0(低耦合)区间内的近似;在该区间之外,ρ*
退化为``存在一条净优势归零边界''这一定性事实(与
§7.2、§9 的限定一致),且该边界的位置只会比独立估计更靠近 ρ=0。
...
\textbf{实证范围总则(适用于 §4.5、§5 与 §7.3--7.6
的全部部署锚点)。}
为免在每处声明重复同一组限定,在此集中声明本文部署实证共享的构念效度边界,后文各处仅在\textbf{偏离}此总则时另作说明:(i)\textbf{单一引擎}------全部实测在
Sui/Mysticeti(commit \texttt{62ee6ada})这一单一 uncertified
引擎上展开,Mysticeti 家族之外无独立协议;(ii)\textbf{代理 certified
臂}------§7.5、§7.6.3 的 certified 对照是同一引擎上一个可开关的可用性证书(2f+1
接受,经 RoundProber 通道观测),\textbf{非}密码学 RBC 签名聚合(其量级外推见
§7.6.4);(iii)\textbf{注入式 Δ\_recover}------涉及单次恢复成本处均使用在恢复路径人为植入的时延旋钮,而非真实
certified 协议的恢复开销;(iv)\textbf{标定而非
benchmark}------因此本文部署所确证的,是各成本轴上 uncertified
一侧的\textbf{形态(方向、量级、缩放律)}与净优势的\textbf{成本结构},而非两套独立协议引擎间的绝对延迟比较。后者(含真实
RBC 开销、跨独立引擎)仍属未来工作(§9、§10)。
...
就统计报告补三点说明。其一(\textbf{为何取 p95}):或有负债是一个\textbf{尾部}现象------良好轮次的代价为零、故障轮次集中爆发,故本文以
cadence 的 p95 而非均值或 p99 为主观测量------均值会被大量良好轮次稀释而掩盖尾部爆发,p99
在每 cell 10--20
次重复的样本量下方差过大、不稳定;p95 在``对尾部敏感''与``在本样本量下可稳定估计''之间取得平衡。其二(\textbf{效应量}):表
7-1 的比值(observed /
expected\_additive)\textbf{本身即一个效应量},其偏离 1
的幅度直接度量超可加强度------5\% 丢包下 n=7 的 +8\%、n=4 的
+12\%,属中等强度的尾部放大,而非仅``边缘显著''。其三(\textbf{精度}):各
cell 比值基于 10--20
次重复,三个关键超可加 cell 另以独立种子完整复测(§9);故可信的是效应的\textbf{方向与量级},而非比值的第三位有效数字。
...
须从一开始就明确本节实证的认识论定位,以免被误读为一次跨协议
benchmark。本节是一次\textbf{成本模型参数的结构性标定}(structural
calibration),经由一个\textbf{代理认证开关}(proxy-certified
toggle)实现,而非两套独立协议的绝对延迟比较。所谓代理认证,指在同一引擎上以一个可开关的\textbf{可用性证书}(被一个
2f+1 quorum
独立接受)来模拟``认证'':它捕获认证的\textbf{结构性效应}------一个独立的确认往返,以及``被引用的祖先已被一个
quorum 持有''------但\textbf{不复现}真实 Bracha RBC 的密码学签名聚合与
O(n²) 广播开销(这两项的量级外推见
§7.6.4)。这一代理设计是有意的:它把``认证强度''隔离为同一引擎上的单一可开关变量,从而以牺牲``绝对延迟可比性''换取``结构性归因的内部效度''。下文给出的全部数值,都应在这一``结构标定而非绝对
benchmark''的意义下理解;其构念效度限定在本节末与 §9 集中声明。
...
为一个 band(由 2 次重复给出的\textbf{范围}而非置信区间,Δ\_recover=500
处有一个噪声离群点),可信的是其量级而非精确值------给出严格的置信区间需要更多独立重复,属未来工作。因此本节标定的是
...
\caption{\textbf{图 7-5} 含代理认证(proxy-certified)对照臂的净优势翻转\textbf{结构性标定}。左:关键路径故障下两臂的恢复停顿 t\_resume 随
...
部署实测,定位为成本结构标定而非绝对 benchmark;cert-gate
为可用性证书(2f+1 接受,非密码学签名),Δ\_recover 为注入式旋钮(真实 RBC
开销外推见 §7.6.4)。}
\end{figure}

\subsubsection{7.6.4 从代理臂到真实 RBC:一个解析外推}\label{sec:rbc-analytic}

§7.6.3 的对照臂是认证的一个\textbf{代理}操作化(可用性证书,2f+1
接受),其 Δ\_save 捕获的是``等 quorum
接受再引用''这一独立确认轮的延迟,而\textbf{不是}真实 Bracha RBC {[}5{]}
连同 BLS 签名聚合 {[}8{]}
的开销。一个自然且必要的追问是:若把代理臂换成真实认证,本节标定的成本结构会朝哪个方向、以多大量级偏移?本节给出一个解析估计来回答它。须先声明其性质:以下是\textbf{量纲分析与数量级估计,而非测量},意在界定
§7.6.3 标定值相对真实认证协议的\textbf{偏置方向},而非给出可直接引用的绝对数字。

记单跳广域网往返为 RTT(地理分布部署典型为
100--250 ms,同区域为数十 ms),验证者数为 n,容错为 f(n≥3f+1)。把真实认证相对代理臂多出的开销拆为三个分量。

\textbf{(1)通信轮次 / 延迟分量。}
代理臂只加回\textbf{一个}独立确认往返(≈1·RTT),实测 Δ\_save≈0.116
s/commit。真实 certified DAG 形成证书需要更多消息时延:Narwhal/Tusk
{[}32{]} 式的可证广播为``提案 → 2f+1 投票 → 聚合并分发证书''约
\textbf{2 个消息时延},而经典 Bracha RBC {[}5{]} 为 send/echo/ready
三阶段、约 \textbf{3 个消息时延}。因此真实认证的 Δ\_save
延迟分量约为代理臂的 2--3 倍(≈0.2--0.35 s/commit
量级)。\textbf{方向明确:代理臂系统性地\emph{低估} Δ\_save}------真实去认证省下的轮次比代理臂测得的更多。

\textbf{(2)密码学 CPU 分量。} BLS {[}8{]} 单次签名约数百微秒,聚合 k 个签名为 k 次群加(可忽略);验证一个针对\emph{同一}区块的聚合签名约
2 次配对(约 1--3 ms),其成本近似常数而不随签名数线性增长。相对一个
RTT,这一 CPU 分量在中小委员会(如 n=7,2f+1=5)下\textbf{可忽略}(亚 ms
至数 ms);仅在大委员会(n≥100,验证成本趋于数十次配对的量级)下才接近一个
RTT。因此在本文 n=7 部署的尺度上,``省去 BLS 验证''并非 Δ\_save
的主要来源------这恰恰\textbf{佐证}了代理臂(不含密码学)在\emph{延迟}维度上是认证延迟的合理代理。

\textbf{(3)消息复杂度 / 带宽分量。} Bracha RBC 的 O(n²)
消息复杂度在 n=7 下约为 49 条/实例(可忽略),但随 n
平方增长:n=100 时约 10⁴
条/实例,在带宽受限或非对称网络下成为 certified 一侧的主要惩罚。这一项正是
§7.5 所述``RBC 自身的 O(n²)
在带宽非对称下压迫低带宽节点''的来源;代理臂(只加一个确认往返、不复制
O(n²) 全广播)\textbf{未能捕获它},因而在大 n 下\emph{低估} certified 一侧的常态成本。

把三项合起来,可得一个清晰的偏置判断。在本文的小委员会尺度(n=7):代理臂对
Δ\_save 给出一个\textbf{保守(偏低)}的延迟估计,而其省略的密码学与 O(n²)
分量均可忽略------因此 §7.6.3 标定的三个定性结论(Δ\_save\textgreater0、认证缩减恢复侧或有负债、存在一个
ρ* 翻转)在外推到真实 RBC 时\textbf{不会反转};其中 Δ\_save
的延迟分量只会被\emph{放大}。至于翻转点 ρ* 的\emph{精确}位置,由于其分子(Δ\_save)与分母侧(认证带来的恢复节省)在外推下\emph{同时}增大,需要真实
benchmark 重新定位;但``存在一条净优势归零边界''这一定性事实不依赖具体外推。在大委员会尺度(n≥100):真实
RBC 的 O(n²)
带宽惩罚会显著抬高 certified 一侧的\textbf{常态}开销,使 Δ\_save 的净值与
ρ*
的位置必须重新标定------这进一步说明 certified/uncertified 的相对优劣是\textbf{规模依赖}的,与
§7.5、§7.6 的条件化结论一脉相承。须重申:以上为解析数量级估计;一个以真实
RBC 签名开销为对照、跨两套独立引擎的受控 benchmark 仍属未来工作(§9、§10)。
...
为使维度二可操作、避免它沦为一个定性标签,本文给出它的一个\textbf{半形式化参数化}。把一个协议的
reconciliation trigger 刻画为一个四元组 T = ⟨G, C, A, B⟩:

\begin{itemize}
\tightlist
\item
  \textbf{触发粒度 G} ∈ \{per-miss, batched(k),
  epoch\}:每发现一个缺失区块即触发、累积 k
  个(或一个时间窗)后批量触发、或按 epoch 边界触发。
\item
  \textbf{触发条件 C} ∈ \{on-reference, on-commit,
  on-timeout(τ)\}:在区块被引用时、被提交时、或超时 τ
  后检测缺失并触发。
\item
  \textbf{主动性 A} ∈ \{pull, push\}:由缺数据的节点主动拉取(pull),还是由持有数据的节点推送补块(push)。
\item
  \textbf{代价承担 B} ∈ \{requester, holder,
  amortized\}:恢复带宽由请求方、持有方、还是全网摊销承担。
\end{itemize}

这一参数化与第 7 章成本模型的耦合是\textbf{显式}的:触发频率 ρ
主要由 (G, C) 决定------``per-miss + on-reference''给出最高的 ρ,而
``batched/epoch + on-commit''显著压低 ρ;单次恢复成本 Δ\_recover
主要由 (A, B) 与 G 决定------push 式补块(如 §7.6.2
观察到的推送式 backlog 绕过关键路径)可把 Δ\_recover 压到近零,而
``pull + requester-pays''在落后节点上集中 Δ\_recover。换言之,维度二不是修辞,而是一个其取值可从协议描述中\textbf{读出}、且可经
(ρ, Δ\_recover) 定量传导到延迟的设计向量。

把这一参数化套到语料库 C,四个协议在维度二上立即分离:Mysticeti
{[}43{]} 为 ⟨per-miss, on-commit, pull,
requester⟩(逐缺失、提交时触发的按需拉取,故 ρ 偏高);Adelie {[}45{]}
的维度一位置与 Mysticeti 相近,却通过区块视图规则在\textbf{上游}限制等价从而压低有效
ρ,相当于在 (G, C) 之前再加一道``少产生缺失''的预防层;Cordial Miners
{[}42{]} 的 blocklace 偏序允许在\textbf{传播阶段}(on-reference)即检测冲突,把
C 前移;DispersedLedger {[}47{]} 则以纠删码编码的承诺把 B 推向
amortized、把 Δ\_recover
压低。这四点在 certified/uncertified 二分下完全不可见(Adelie 与 Mysticeti
同属 uncertified),在维度二的参数化下却清晰可分------这正是维度二独立解释力的直接体现(§8.4
进一步展开)。
...
\subsection{8.5 深层轴的一个延伸猜想:availability-enforcement
与排序公平性}\label{ux6df1ux5c42ux8f74ux7684ux5ef6ux4f38availability-enforcement-ux4e0eux6392ux5e8fux516cux5e73ux6027}

最后,本文以一个\textbf{明确标注为猜想(conjecture)、而非已论证结论}的延伸收尾,以示意深层轴的影响\textbf{或许}不止于延迟。DAG
共识的排序规则建立在 DAG 结构之上,而 DAG
结构由哪些区块可用、何时可用所塑造------由此自然浮现一个\textbf{待验证的机制猜想}:availability-enforcement
时机越靠后(post-commit),一个区块``是否及何时进入
DAG''便越晚、越依赖运行时条件被决定,从而\textbf{可能}给拜占庭节点更大的、操纵``排序时哪些区块可见''的空间;而在
pre-reference(certified)下,区块在作者观察到排序态势之前即被锁定,该操纵选项被关闭。本文\textbf{不}就此主张任何结论,也\textbf{不}以其支撑贡献
C:此处既未量化敌手可获取的 MEV
{[}53{]}--{[}56{]},未给出任何防护下界,也无任何实证。其唯一作用,是示意本文坐标系\textbf{有能力生成}延迟之外的、可证伪的研究问题------这一生成性本身,是深层轴作为分析框架的一个旁证。该猜想的形式化与检验留作未来工作(§10)。
...
RBC 签名开销、跨两套独立引擎的受控
benchmark,以从``成本结构标定''推进到``真实协议对的绝对延迟比较''。此外,§8.5
提出的延伸猜想------把 availability-enforcement
时机视为一个``排序可操纵性''维度------其形式化、敌手可获取
MEV 的量化与防护下界的给出,是深层轴在延迟之外的一个可证伪方向,同样留作未来工作。
...
\section{附录 A:复现指南}\label{sec:repro}

本附录把复现流程集中为一处,与上文``数据可用性声明''互补。全部部署材料以
commit \texttt{62ee6ada}(MystenLabs/sui,main 分支,2026-05-22)为基线。

\textbf{A.1 环境前提。}
(i)编译论文------TeX 发行版(MiKTeX/TeX Live)+ XeLaTeX + Windows
字体(SimSun、SimHei、KaiTi、Consolas);(ii)解析模型与仿真------Python 3
加 \texttt{simpy}、\texttt{numpy}、\texttt{matplotlib};(iii)部署实证------Docker
Desktop + PowerShell,并以 \texttt{git clone} 后 \texttt{git checkout
62ee6ada} 获取 Sui 源码(vendor 源码不随本仓库分发)。
...
\tightlist
\item
  \textbf{解析成本模型与图 1--3}(第 7--8 章):在
  \texttt{submit/simulation/} 下,\texttt{run\_sweep.py} 与
  \texttt{run\_calibrated\_sweep.py} 生成成本台账与参数扫描,\texttt{run\_verify.py}
  复现 §7.2 的 Monte Carlo 实现核对(2000 轮 / 500
  trial)。图 1--3 为该解析模型的数值/定性图示,非真实协议实测。
\item
  \textbf{部署或有负债实证}(§4.5、§5、§7.3--7.6.1):在
  \texttt{submit/deployment/multivalidator/} 下,按 \texttt{config/}
  中的实验矩阵运行 \texttt{scripts/} 下对应 PowerShell
  脚本;逐 run 指标落入 \texttt{data/per\_run\_metrics/}(503
  个目录),按场景汇总落入 \texttt{data/runs/}。
\item
  \textbf{代理认证对照臂标定}(§7.6.2--7.6.3):在
  \texttt{submit/calibration/} 下,\texttt{scripts/phaseA\_recover\_calibration.ps1}
  对应 §7.6.2 的 Δ\_recover 旋钮标定,\texttt{scripts/phase2c\_excess\_sweep.ps1}
  对应 §7.6.3 的 certificate-gate excess 扫描;Route A1 共识源码改动与结果见
  \texttt{PHASE1\_RESULTS.md}、\texttt{PHASE2\_RESULTS.md}。
\end{itemize}

\textbf{A.3 图--脚本--数据对应。}
每张部署图均可由一个汇总 CSV 与一个分析脚本重建,主要对应:图
4-1←\texttt{payload\_size\_summary.csv};图
5-1←\texttt{chapter5\_enforcement\_summary.csv};图
5-2/5-3←\texttt{partition\_grid\_summary.csv};图
7-6←\texttt{rho\_scan\_summary.csv};图
7-7←\texttt{gst\_jitter\_summary.csv};图
7-8←\texttt{net\_asymmetry\_summary.csv};图 7-3 与表
7-1←\texttt{phaseA\_criticalpath\_summary.csv} 及
\texttt{phaseA\_criticalpath\_n4\_summary.csv};图
7-5←\texttt{phase2c\_excess\_sweep\_summary.csv} 与
\texttt{phase1b\_dsave\_calibration\_summary.csv}。统一的图表生成入口为
\texttt{submit/calibration/scripts/generate\_figures.py};各分析脚本(如
\texttt{analyze\_payload\_size.py})与汇总 CSV 同名对应。

\textbf{A.4 数据完整性事件。} 两起已诚实记录的数据完整性事件(P0-1
\texttt{timeout.exe} 伪迹致 recFetch 误记为 0;P0-2 ρ
首轮近恒定的设计缺陷)及其受污染数据,存档于
\texttt{submit/calibration/EXPERIMENTAL\_COVERAGE.md},以便审查者核验修复前后的差异。
...
\bibitem{ref41} N. Shrestha and A. Kate, “Towards improving throughput and scalability of DAG-based BFT SMR,” in \textit{Proc. 21st European Conf. Computer Systems (EuroSys)}, 2026. (Also IACR Cryptol. ePrint Arch., Paper 2025/877.)
...
\bibitem{ref48} M. Al-Bassam, A. Sonnino, V. Buterin, and I. Khoffi, “Fraud and data availability proofs: Detecting invalid blocks in light clients,” in \textit{Proc. Financial Cryptography and Data Security (FC)}, 2021.
